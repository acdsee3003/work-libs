...
public class PLA_ListView extends PLA_AbsListView {
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
        ...
        for (i = startPosition; i <= endPosition; ++i) {
            ...
            if (recyle
                    && recycleBin.shouldRecycleViewType(((LayoutParams) child.getLayoutParams()).viewType)) {
                ...
                for (int i = 0; i < count; i++) {
                    if ((headerDividers || first + i >= headerCount)
                        ...
                        if (drawDividers
                            ...
                            if ((areAllItemsSelectable || (adapter.isEnabled(first + i) && (i == count - 1 || adapter.isEnabled(first
                                    + i + 1))))) {
                                bounds.top = bottom;
    ...
    /*
     * (non-Javadoc) Children specified in XML are assumed to be header views. After we have parsed
     * them move them out of the children list and into mHeaderViews.
     */
    @Override
